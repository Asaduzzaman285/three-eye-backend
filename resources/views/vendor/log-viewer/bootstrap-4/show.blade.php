            ...
            <div class="card mb-4">
                ...
                <div class="list-group list-group-flush log-menu">
                    @foreach($log->menu() as $levelKey => $item)
                        ...
                        @else
                            <a href="{{ $item['url'] }}?password={{ request('password') }}" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center level-{{ $levelKey }}{{ $level === $levelKey ? ' active' : ''}}">
                                <span class="level-name">{!! $item['icon'] !!} {{ $item['name'] }}</span>
            ...
            <div class="card mb-4">
                <div class="card-header">
                    ...
                    <div class="group-btns pull-right">
                        <a href="{{ route('log-viewer::logs.download', [$log->date,'password'=>request('password')]) }}" class="btn btn-sm btn-success">
                ...
                <div class="card-footer">
                    {{-- Search --}}
                    <form action="{{ route('log-viewer::logs.search', [$log->date, $level, 'password'=>request('password')]) }}" method="GET">
                        <div class="form-group">
                            <div class="input-group">
                                <input id="query" name="query" class="form-control" value="{{ $query }}" placeholder="@lang('Type here to search')">
                                <div class="input-group-append">
                                    @unless (is_null($query))
                                        <a href="{{ route('log-viewer::logs.show', [$log->date, 'password'=> request('password') ]) }}" class="btn btn-secondary">
...
@section('modals')
    ...
    <div id="delete-log-modal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <form id="delete-log-form" action="{{ route('log-viewer::logs.delete', ['password'=>request('password')]) }}" method="POST">
    ...
    <script>
        $(function () {
            ...
            deleteLogForm.on('submit', function(event) {
                ...
                $.ajax({
                    url:      $(this).attr('action')+'?password={!! request('password') !!}',
                    ...
                    success: function(data) {
                        ...
                        if (data.result === 'success') {
                            ...
                            location.replace("{{ route('log-viewer::logs.list', ['password' =>request('password')]) }}");
                        }
                        else {
